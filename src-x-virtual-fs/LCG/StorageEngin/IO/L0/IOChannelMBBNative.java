...
@Deprecated
public class IOChannelMBBNative implements IOInterface{ 
    static {
    	try{ 
    		...
    		System.load(abs_path + "/libLinux_X86_32_IOL0_Native.so");
    		//System.load("/home/feiben/eclipsWorkspace/StorageEnginCPP/Debug/libLinux_X86_32_IOL0_Native.so");
    	} catch(UnsatisfiedLinkError e) 
    	{ 
            System.err.println( "Cannot load Linux_X86_32_IOL0_Native library:\n " + 
            		e.toString() ); 
    ...
    protected int _physical_mem_addr;
    private boolean closed;
    private boolean terminated;
    private String _name;

    public IOChannelMBBNative(String name, String mode, int bufbitlen) throws IOException 
    { 
    ...
    public native int initMBB(String name, String mode, int bufbitlen); 
    public native void closeMBB(int physical_mem_addr);
    private native void terminator(int physical_mem_addr);

    public native boolean	isReadOnly(int physical_mem_addr); 
    public native boolean	flush(int physical_mem_addr);
    /*
    ...
    public native long		GetCurrentPos(int physical_mem_addr);
    public native boolean	seek(int physical_mem_addr, long pos);
    public native long		length(int physical_mem_addr);
    public native boolean	seekEnd(int physical_mem_addr);
    public native boolean	setLength(int physical_mem_addr, long new_length);
    public native int		readByteArray(int physical_mem_addr, byte b[], int off, int len);
    public native boolean	writeByteArray(int physical_mem_addr, byte b[], int off, int len);
    public native int		readIntArray(int physical_mem_addr, int b[], int off, int len);
    public native boolean	writeIntArray(int physical_mem_addr, int b[], int off, int len);
    
    private native byte		readByte(int physical_mem_addr);
    private native byte		readByteAt(int physical_mem_addr, long pos);
    public native boolean	writeByte(int physical_mem_addr, byte bw);
    private native boolean	writeByteAt(int physical_mem_addr, byte bw, long pos);
	
    /*
     * variable utility functions
     */
    public native boolean	WriteLong(int physical_mem_addr, long i);
    public native long		ReadLong(int physical_mem_addr);
    public native boolean	write1ByteInt(int physical_mem_addr, int i);
    public native int		read1ByteInt (int physical_mem_addr);
    public native boolean	write4ByteInt(int physical_mem_addr, int i);
    public native int		read4ByteInt(int physical_mem_addr);
